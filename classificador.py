...
class SVM(Classificador):
    ...
    def criar(self):
        ...
        hyper = {"C": [1.0, 2.0, 3.0],
                 ...
                 "degree": [2, 3, 4],
                 "gamma": ["scale"]}
        revisoes_treino, self.revisoes_teste, recomendacoes_treino, self.recomendacoes_teste = train_test_split(
            revisoes_processadas, self.recomendacoes, test_size=0.2, random_state=0)
        kfold = StratifiedKFold(n_splits=10, random_state=1, shuffle=True)
        classificador = GridSearchCV(self.classificador, hyper, cv=kfold, n_jobs=1)
        results.append(classificador)
